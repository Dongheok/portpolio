    ...
    const setLayout = () => {
      ...
      for (let i = 0; i < sceneInfo.length; i++) {
        if (sceneInfo[i].type === "sticky") {
          ...
          // !!
          sceneInfo[i].scrollHeight = window.innerHeight + 300;
        }
        sceneInfo[
          i
    ...
    const playAnimation = () => {
      ...
      switch (currentScene) {
        ...
        case 1:
          ...
          if (scrollRatio <= 0.4) {
            objs.logo.current.style.width = `${calcValues(
              values.logo_width_in,
              currentYOffset
            )}vw`;
            objs.logo.current.style.transform = `translate(${calcValues(
              values.logo_translateX_in,
              currentYOffset
            )}%, -50%)`;
          } else {
            objs.logo.current.style.width = `${calcValues(
              values.logo_width_out,
              currentYOffset
            )}vw`;
            objs.logo.current.style.transform = `translate(${calcValues(
          ...
          // 빨간 리본 패스(줄 긋기)
          if (scrollRatio <= 0.5) {
            objs.ribbonPath.current.style.strokeDashoffset = calcValues(
          ...
          objs.logo.current.style.opacity = calcValues(
            values.logo_opacity_out,
            currentYOffset
          );
